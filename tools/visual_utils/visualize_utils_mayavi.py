...
def visualize_pts(pts, fig=None, bgcolor=(1.0, 1.0, 1.0), fgcolor=(1.0, 1.0, 1.0),
    ...
    if pts.shape[1]==4:
        mlab.points3d(pts[:, 0], pts[:, 1], pts[:, 2], pts[:, 3], scale_mode="none", mode='sphere',
                          colormap='gnuplot', scale_factor=0.1, figure=fig)
    else:
        d = np.sqrt(pts[:, 0] ** 2 + pts[:, 1] ** 2)  # Map Distance from sensor
        mlab.points3d(pts[:, 0], pts[:, 1], pts[:, 2], d, scale_mode="none", mode='sphere',
                          colormap='gnuplot', scale_factor=0.1, figure=fig)
    if draw_origin:
...
def draw_scenes(points, pred_boxes=None, gt_boxes=None, ref_boxes=None, ref_scores=None, ref_labels=None):
    ...
    if pred_boxes is not None:
        corners3d = boxes_to_corners_3d(pred_boxes)
    ...
    mlab.view(azimuth=-180, elevation=1, distance=50.0, roll=90.0)
    return fig
