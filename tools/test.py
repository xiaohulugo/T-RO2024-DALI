...
from eval_utils import eval_mine_utils
...
def eval_single_ckpt(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=False,scale=1.0):
    # load checkpoint
    model.load_params_from_file(filename=args.ckpt, logger=logger, to_cpu=dist_test)
    model.cuda()

    # start evaluation
    eval_mine_utils.eval_one_epoch(
        cfg, model, test_loader, epoch_id, logger, dist_test=dist_test,
        result_dir=eval_output_dir, save_to_file=args.save_to_file, args=args, scale=scale
    )
...
def repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=False):
    ...
    while True:
        ...
        tb_dict = eval_mine_utils.eval_one_epoch(
...
def main():
    ...
    test_set, test_loader, sampler = build_dataloader(
        ...
        dist=dist_test, workers=args.workers, logger=logger, training=False,feat=cfg.CAD3D_CONFIG.FEATURE
    )
    model = build_network(model_cfg=cfg.MODEL, num_class=len(cfg.DATA_CONFIG_TAR.CLASS_NAMES), dataset=test_set)
    ...
    with torch.no_grad():
        if args.eval_all:
            ...
            eval_single_ckpt(model, test_loader, args, eval_output_dir, logger,
                            epoch_id, dist_test=dist_test, scale=args.scale)
