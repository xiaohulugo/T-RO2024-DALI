...
class VoxelSetAbstraction(nn.Module):
    ...
    def point_to_bev(self, points, bev_stride):
        x_idxs = (points[:, 0] - self.point_cloud_range[0]) / self.voxel_size[0]
        y_idxs = (points[:, 1] - self.point_cloud_range[1]) / self.voxel_size[1]
        x_idxs = x_idxs / bev_stride
        y_idxs = y_idxs / bev_stride
        xy = torch.stack([y_idxs, x_idxs]).view(-1,2)
        return xy
    ...
    def forward(self, batch_dict):
        ...
        point_bev_yx = self.point_to_bev(point_coords[:,1:4], batch_dict['spatial_features_stride']  )
        ...
        batch_dict['point_bev_yx'] = point_bev_yx.type(torch.long)  # (BxN, 2)
        return batch_dict
