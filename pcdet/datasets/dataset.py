...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None, feat='xyz'):
        super().__init__()
        ...
        #self.db_path = Path(self.dataset_cfg.DB_PATH)
        self.logger = logger
        self.feat = feat
        self.zshift = 0.0
        self.scale = 1.0
        ...
        if self.training and self.dataset_cfg.get('DATA_AUGMENTOR', None) is not None:
        ...
        self.data_processor = DataProcessor(
            ...
            num_point_features=self.point_feature_encoder.num_point_features, feat=self.feat
        )
    ...
    def prepare_data(self, data_dict):
        ...
        if self.training and self.data_augmentor is not None:
            ...
            if num_points_in_gt is None:
                mask_inbox = roiaware_pool3d_utils.points_in_boxes_cpu(
                    torch.from_numpy(data_dict['points'][:, :3]),
                    torch.from_numpy(data_dict['gt_boxes'][:, :7])).numpy()
                num_points_in_gt = np.sum(mask_inbox, axis=1)
                mask = (num_points_in_gt >= self.dataset_cfg.get('MIN_POINTS_OF_GT', 1))
                data_dict['gt_boxes'] = data_dict['gt_boxes'][mask]
                data_dict['gt_names'] = data_dict['gt_names'][mask]

                mask_remove = np.zeros(mask_inbox.shape[1])
                for i in range(mask_inbox.shape[0]):
                    if not mask[i]:
                        mask_remove += mask_inbox[i]
                data_dict['points'] = data_dict['points'][mask_remove==0]
        ...
        if data_dict.get('gt_boxes', None) is not None:
            # if isinstance(data_dict['gt_names'], list): 
            #     data_dict['gt_names'] = np.array(data_dict['gt_names'])            
            #selected = data_dict['gt_names']==self.class_names
        ...
        if self.training and (self.data_augmentor is not None) and len(data_dict['gt_boxes']) == 0:
