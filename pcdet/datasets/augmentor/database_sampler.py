...
class DataBaseSampler(object):
    ...
    def update(self):
        ...
        for db_info_path in self.sampler_cfg.DB_INFO_PATH[0]:
            db_info_path = self.root_path.resolve() / db_info_path
            with open(str(db_info_path), 'rb') as f:
                infos = pickle.load(f)
                [self.db_infos[cur_class].extend(infos[cur_class]) for cur_class in self.class_names]
                print(len(infos[self.class_names[0]]))
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict):
        ...
        for i in range(len(total_valid_sampled_dict)):
            info = total_valid_sampled_dict[i]
            file_path = self.root_path / info['path']
            obj_points = np.fromfile(str(file_path), dtype=np.float32).reshape(
                [-1, self.sampler_cfg.NUM_POINT_FEATURES])
            # obj_points = np.load(str(file_path))
            # obj_points = np.nan_to_num(obj_points, nan=0)
            #obj_points[:,0:3] /= self.sampler_cfg.SCALE
            # if obj_points.shape[0]>100:
            ...
            #lwh /= self.sampler_cfg.SCALE
            xyz[2] = z_ground+lwh[2]/2.0
            sampled_gt_boxes[i,0:3] = xyz            
            sampled_gt_boxes[i,3:6] = lwh   
            obj_points[:, :3] += xyz
        ...
        if obj_points.shape[1]!=points.shape[1]:
            obj_points = np.concatenate([obj_points, np.zeros((obj_points.shape[0],points.shape[1]-obj_points.shape[1]))], axis=1)            
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                sampled_boxes = sampled_boxes[:,0:7]
