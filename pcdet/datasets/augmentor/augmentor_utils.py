...
def global_sampling(gt_boxes, points, gt_boxes_mask, sample_ratio_range, prob, th_pts):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading]
        points: (M, 3 + C)
...
def scale_pre_object(gt_boxes, points, gt_boxes_mask, scale_perturb, num_try=50):
    ...
    return points, gt_boxes, gt_boxes_mask
...
def drop_points(gt_boxes, points, gt_boxes_mask, th_points=100):
    """
    ...
    point_idx = np.arange(0, num_pts, 1, dtype=int)
    point_masks_all = np.ones(num_pts)

    for idx in range(num_boxes):
        # don't need to rotate this object
        if not gt_boxes_mask[idx]:
            continue

        point_masks = roiaware_pool3d_utils.points_in_boxes_cpu(points[:, 0:3],
                                                                np.expand_dims(gt_boxes[idx], axis=0)).squeeze(0)
        if np.sum(point_masks)>th_points:
            idx_pts_inbox = point_idx[point_masks>0]
            num_pts_inbox = idx_pts_inbox.shape[0]
            num_pts_keep = int(num_pts_inbox*random.randint(3,10)/10)
            idx_dropped = random.sample(range(0, num_pts_inbox), num_pts_inbox-num_pts_keep)
            point_masks_all[idx_pts_inbox[idx_dropped]] = 0

    points = points[point_masks_all>0]
