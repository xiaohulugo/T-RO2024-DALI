...
class KittiDataset(DatasetTemplate):
    ...
    def update_ps_label(self, path):        
        if self.dataset_cfg.get('USE_PSEUDO_LABEL', None) and self.training:
            ps_file = open(path, "rb")
            self.ps_labels_dict = pickle.load(ps_file)
            ps_file.close() 
        count_box = 0
        for key in self.ps_labels_dict:
            self.ps_labels_dict[key] = self.ps_labels_dict[key].cpu().numpy()
            count_box += self.ps_labels_dict[key].shape[0]
        print(count_box)
