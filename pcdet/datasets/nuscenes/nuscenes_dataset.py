...
class NuScenesDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, feat='xyz'):
        ...
        if self.dataset_cfg.get('CLASS_MAP', False):
            self.class_map = dataset_cfg.CLASS_MAP
        else:
            self.class_map = None

        self.infos = []
        self.include_nuscenes_data(self.mode, self.class_map)
        if self.training and self.dataset_cfg.get('BALANCED_RESAMPLING', False):
            self.infos = self.balanced_infos_resampling(self.infos)


    def include_nuscenes_data(self, mode, class_map):
            ...
            with open(info_path, 'rb') as f:
                ...
                if class_map is not None:
                    for i in range(len(infos)):
                        class_list = infos[i]['gt_names']
                        for j in range(len(class_list)):
                            if class_list[j] in class_map:
                                class_list[j] = class_map[class_list[j]]
    ...
    def __getitem__(self, index):
        ...
        if 'gt_boxes' in info:
            ...
            #input_dict['gt_boxes'][:, 0:6] *= self.scale
            
 #               px = copy.deepcopy(input_dict['gt_boxes'][:, 0])
 #               input_dict['gt_boxes'][:, 0] = input_dict['gt_boxes'][:, 1]
 #               input_dict['gt_boxes'][:, 1] = -px        
            
            #input_dict['gt_boxes_ori'] = copy.deepcopy(input_dict['gt_boxes'])
...
if __name__ == '__main__':
    ...
    # --cfg_file /home/lxh/Documents/ST3D/ST3D/tools/cfgs/dataset_configs/nuscenes_dataset.yaml 
    # --version v1.0-trainval --root '/media/lxh/New Volume/Dataset/nuScenes'
    #
    parser = argparse.ArgumentParser(description='arg parser')
    parser.add_argument('--cfg_file', type=str, default=None, help='specify the config of dataset')
    parser.add_argument('--func', type=str, default='create_nuscenes_infos', help='')
