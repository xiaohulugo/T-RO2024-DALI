...
class WaymoDataset(DatasetTemplate):
    ...
    def create_groundtruth_database_parallel(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
                                             processed_data_tag=None, num_workers=8, crop_gt_with_tail=False):
        use_sequence_data = self.dataset_cfg.get('SEQUENCE_CONFIG', None) is not None and self.dataset_cfg.SEQUENCE_CONFIG.ENABLED
        if use_sequence_data:
            st_frame, ed_frame = self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET[0], self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET[1]
            self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET[0] = min(-4, st_frame)  # at least we use 5 frames for generating gt database to support various sequence configs (<= 5 frames)
...
def create_waymo_infos(dataset_cfg, class_names, data_path, save_path,
                       ...
                       workers=min(8, multiprocessing.cpu_count()), update_info_only=False):
...
def create_waymo_gt_database(
    ...
    workers=min(8, multiprocessing.cpu_count()), use_parallel=False, crop_gt_with_tail=False):
    dataset = WaymoDataset(
        dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path,
        training=False, logger=common_utils.create_logger()
    )
...
if __name__ == '__main__':
    # a = np.load('/media/lxh/Data/Waymo_V1.2/waymo_processed_data_v0_5_0/segment-14004546003548947884_2331_861_2351_861_with_camera_labels/0188.npy')
    # b = 0
    import argparse
    import yaml
    ...
    args.cfg_file = '/egr/research-canvas/detection3d_codes/GPS3D/tools/cfgs/configs/da_waymo_dataset.yaml'
    args.func = 'create_waymo_infos'
    args.processed_data_tag = 'waymo_processed_data_v0_5_0'

    #ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
    ROOT_DIR = Path('/egr/research-canvas/detection3d_datasets/waymo_v1')
